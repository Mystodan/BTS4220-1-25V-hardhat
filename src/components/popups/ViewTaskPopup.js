 ...
 */
const ViewTaskPopup = ({ task, onClose, usernames }) => {
    if (!task) return null;
    return (
        <Popup
            open={!!task}
        ...
        >
            <div>
                <b>ID:</b> {String(task.id)}
                <br />
                <b>Content:</b> {task.content}
                <br />
                <b>Private:</b> {task.is_private ? "Yes" : "No"}
                <br />
                <b>Created by:</b> {task.user}
                <br />
                {task.user && usernames && usernames[task.user] ? (
                    <>
                        <b>Username:</b> {usernames[task.user]}
                        <br />
                    </>
                ) : null}
                ...
                <br />
                <b>Created At:</b>{" "}
                {task.createdAt && !isNaN(task.createdAt)
                    ? new Date(task.createdAt * 1000).toLocaleString()
                    : "-"}
                <br />
                <b>Completed At:</b>{" "}
                {task.completedAt &&
                !isNaN(task.completedAt) &&
                task.completedAt !== 0
